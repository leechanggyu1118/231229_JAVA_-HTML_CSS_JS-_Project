...
public class ArrayEX01 {
	...
	public static void main(String[] args) {
		...
		//(int)(Math.random()*10)+0; //0부터 10개의 랜덤 수 출력
		//max = 최종번지 개수, min = 첫 번지
		//교환 코드
		//[i][i+1]
//		int temp = arr[i];
//		arr[i] = arr[i+1];
//		arr[i+1] = temp;
		...
		for(int i = 0;i<arr.length;i++) {
			int random = (int)(Math.random()*max)+min; //랜덤한 번지값
			int temp = arr[i];
			arr[i] = arr[random];
			arr[random] = temp;
		}
		...
		//작은 수 부터 정리 = 오름자순 정렬
		//큰 수 부터 정리 = 내림차순 정렬
		// 3 2 5 6 7 1 9 4 10 8 - 버블정렬 해당 번지는 고정 뒤에 있는 번지의 값과 비교 후 정렬
		// 1 3 5 6 7 2 9 4 10 8 - paas1
